...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `DASHSCOPE_API_KEY` + `ZENMUX_API_KEY` | 转录 + 语义剪辑，独立引用、同页填写 |
| transcription | `DASHSCOPE_API_KEY` | `screen-studio-editor/dashscope/default` |
| analysis | `ZENMUX_API_KEY` | `screen-studio-editor/zenmux/default` |
| native | `GEMINI_API_KEY` | `screen-studio-editor/gemini/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- "$SKILL_DIR/.venv/bin/python3" "$SKILL_DIR/scripts/smart_edit_workflow.py" --project "/path/to/Project.screenstudio"
```
...
默认质量剪辑通过 default 同页配置转录与分析服务；只清理停顿并使用云端 ASR 时使用 transcription，不要求分析 Key。本地合并等不调用服务的操作不需要新增 Key。依赖的 bl CLI 仍须确认已安装，程序继承 `DASHSCOPE_API_KEY`，不执行登录写入或把 Key 放到参数。

分析默认适配器为 ZenMux，转录为 DashScope；native 仅用于用户明确选择 `gemini_edit_candidates.py --review-backend gemini` 的直接 API 路径，不代表整个质量流水线已支持直连。服务会接收对齐后的音视频或候选证据，可能计费。原配置只兼容读取，不自动迁移或删除。
